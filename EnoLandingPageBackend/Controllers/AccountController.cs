namespace EnoLandingPageBackend.Controllers {
  ...
  public class AccountController : ControllerBase {
    ...
    /// Login to the Portal.
    /// </summary>
    /// <param name="redirectUri">The URL the User will be redirected after
    /// succesfull login.</param> <returns></returns>
    [HttpGet]
    public ActionResult
    Login(string redirectUri) // TODO 404 foo makes ReturnUrl out of this
    {
      return this.Challenge(
          new AuthenticationProperties(){
              RedirectUri =
                  $"/api/account/oauth2redirect?redirectUri={HttpUtility.UrlEncode(redirectUri)}",
          },
          "ctftime.org");
    }
    ...
    public async Task<ActionResult> OAuth2Redirect(string redirectUri) {
      var ctftimeIdClaim =
          this.HttpContext.User.FindFirst(LandingPageClaimTypes.CtftimeId)
          ?.Value;
      var teamname = this.HttpContext.User.Identity?.Name;
      if (!long.TryParse(ctftimeIdClaim, out long ctftimeId) ||
          teamname == null) {
        throw new Exception(
            $"OAuth2 failed: ctftimeid={ctftimeIdClaim} teamname={teamname} claims={this.HttpContext.User.Claims.Count()}");
      }

      if (DateTime.UtcNow > this.settings.GetRegistrationCloseTime() &&
          !await this.db.CtftimeTeamExists(ctftimeId,
      ...
      try {
        info = await CTFTime.GetTeamInfo(ctftimeId,
                                         this.HttpContext.RequestAborted);
      } catch (Exception e) {
        this.logger.LogError(
            $"CTFtime failed to deliver info for ctftime id {ctftimeId}\n{e.StackTrace}");
      }

      var team = await this.db.GetOrUpdateLandingPageTeam(
          ctftimeId, teamname, info?.Logo, null, info?.Country,
          this.HttpContext.RequestAborted);
    ...
    public async Task<ActionResult<TeamDetailsMessage>> Info() {
      var team = await this.db.GetTeamAndVulnbox(
          this.GetTeamId(), this.HttpContext.RequestAborted);
      return this.Ok(new TeamDetailsMessage(
          team.Id, team.Confirmed, team.Name,
          team.Vulnbox.ExternalAddress != null, // vpnconfig available
          team.Vulnbox.RootPassword, team.Vulnbox.ExternalAddress,
    ...
    public async Task<ActionResult> VpnConfig() {
      ...
      var config = System.IO.File.ReadAllText(
          $"{LandingPageBackendUtil.TeamDataDirectory}{Path.DirectorySeparatorChar}teamdata{Path.DirectorySeparatorChar}team{team.Id}{Path.DirectorySeparatorChar}client.conf");
      var contentType = "application/force-download";
      return this.File(
          Encoding.ASCII.GetBytes(config.Replace("REMOTE_IP_PLACEHOLDER",
                                                 team.Vulnbox.ExternalAddress)),
          contentType, "client.conf");
    }
